...
{
    /// <summary>
    /// Custom control đ? hi?n th? voice message trong FlowLayoutPanel
    /// </summary>
    public class VoiceMessageBubble : UserControl
    {
        ...
        private void InitializeComponents()
        {
            ...
            lbDuration = new Label
            {
                ...
                Text = $"Thời gian: {voiceMessage.Duration}s"
            };
        ...
        private void LoadContent()
        {
            try
            {
                ...
                {
                    lbTranscription.Text = $"[File không tìm thấy: {voiceMessage.TranscriptionFileName}]";
                    Debug.WriteLine($"[VOICE UI] Transcription file not found: {transcriptionPath}");
                }
            }
            catch (Exception ex)
            {
                lbTranscription.Text = $"[Lời: {ex.Message}]";
        ...
        private void BtnPlay_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show($"File không tìm thấy: {audioPath}", "Lời");
                ...
                MessageBox.Show($"Đang phát: {voiceMessage.AudioFileName}", "Thông báo");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lời phát âm thanh: {ex.Message}", "L?i");
